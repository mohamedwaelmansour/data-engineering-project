...
try:
    print("Reading Data from Bronze Parquet Layer...")
    df_raw = spark.read.parquet(BRONZE_PATH)
    
    print(f"Total records found in Bronze: {df_raw.count()}")
    ...
    print("Building Star Schema Layers (Dimensions & Facts)...")
    ...
    print("Saving Star Schema to Gold Layer (Idempotent Overwrite)...")
